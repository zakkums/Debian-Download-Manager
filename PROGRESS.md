...
- [x] **All source files &lt;200 lines** – `scheduler/execute/mod.rs` (202→150): added `setup.rs` (storage/budget/progress) and `invoke.rs` (spawn_blocking download). `safe_resume/validate.rs` (230→55): became `validate/mod.rs` + `validate/error.rs` + `validate/tests.rs`. `cli/tests.rs` (213→88+127): became `cli/tests/mod.rs` + `add_run.rs` + `rest.rs`. No code or behaviour changed; tests pass.
